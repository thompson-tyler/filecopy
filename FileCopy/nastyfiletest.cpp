...
void copyFile(string sourceDir, string fileName, string targetDir,
...
int main(int argc, char *argv[]) {
    ...
    if (argc != 4) {
        fprintf(
            stderr,
            "Correct syntxt is: %s <nastiness_number> <SRC dir> (TARGET dir>\n",
            argv[0]);
        exit(1);
    }
...
void copyFile(string sourceDir, string fileName, string targetDir,
    ...
    if (!isFile(sourceName)) {
        cerr << "Input file " << sourceName
             << " is a directory or other non-regular file. Skipping" << endl;
        return;
    }
